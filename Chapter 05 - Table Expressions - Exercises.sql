...
DECLARE @SequenceLength INT = 64;

WITH Fibonacci AS (
  SELECT
    1 AS Number
    , CONVERT(BIGINT, 1) AS Previous
    , CONVERT(BIGINT, 1) AS Fibonacci
  UNION ALL
...
SELECT orderid, orderdate, custid, empid, COUNT AS rownum
FROM Sales.Orders;
