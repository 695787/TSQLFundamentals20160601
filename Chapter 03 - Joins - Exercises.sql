...
-- My Answer
  SELECT H.empid, DATEADD(day, n-1, CAST('20160612' AS DATE)) AS orderdate
     FROM dbo.Nums
...
-- My Answer- Potential Answer- Workings
...
SELECT C.custid, COUNT(OD.orderid) AS numorders, OD.qty AS totalqty
FROM Sales.Customers AS C
  LEFT OUTER JOIN
      (Sales.Orders AS O
         INNER JOIN Sales.OrderDetails AS OD
...
SELECT O.custid, COUNT(OD.orderid) AS numorders
FROM Sales.Customers AS C
  LEFT OUTER JOIN
      (Sales.Orders AS O
         INNER JOIN Sales.OrderDetails AS OD
...
-- My Answers- Different from given answer- totalqty seems to be the same.
